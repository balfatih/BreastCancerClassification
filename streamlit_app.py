...
st.title("Breast Cancer Classification (Using Precomputed Features & GBM)")
...
@st.cache_data
def load_features():
    x_test = np.load("x_test.npy")  # shape: (num_samples, feature_dim)
...
if uploaded_file is not None:
    ...
    # Görüntüyü 128x128 ve normalize et
    IMG_SIZE = 128
    img_array = np.array(image.resize((IMG_SIZE, IMG_SIZE))).astype("float32") / 255.0
    img_array = img_array.flatten().reshape(1, -1)  # Flatten ve (1, feature_dim)

    # ----------------------
    # En yakın feature bulma (cosine similarity)
    # ----------------------
